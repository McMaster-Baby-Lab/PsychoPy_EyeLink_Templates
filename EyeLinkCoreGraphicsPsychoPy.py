...
class EyeLinkCoreGraphicsPsychoPy(pylink.EyeLinkCustomDisplay):
    def __init__(self, tracker, win):
        ...
        if not DISABLE_AUDIO:
            try:
                self._target_beep = Sound(os.path.join('sounds', 'type.wav'), stereo=True)
                self._error_beep = Sound(os.path.join('sounds', 'error.wav'), stereo=True)
                self._done_beep = Sound(os.path.join('sounds', 'qbeep.wav'), stereo=True)
